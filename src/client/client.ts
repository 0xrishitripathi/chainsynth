...
const rpcEndpoint = 'wss://mainnet.avail-rpc.com/';
...
class ChainSynth {
    ...
    private async processBlockDetails(header: Header) {
        ...
        let block;
        try {
            block = await this.substrateClient.rpc.chain.getBlock(header.hash);
        } catch (err) {
            console.warn('Unable to retrieve block from supplied hash, falling back to latest block:', err);
            // Fallback prevents a hard-crash if the node prunes the requested block or
            // the runtime metadata for that specific hash is unavailable (common on Avail).
            block = await this.substrateClient.rpc.chain.getBlock();
        }
        this.ui.blockNumber.innerHTML =
            `<a href="https://avail.subscan.io/block/${header.number.toString()}" target="_blank">#${numberFormat
                .format(header.number.toNumber())
                .replace(/,/g, '_')}</a>`;
        ...
        try {
            ...
            console.warn('Unable to retrieve registry via hash, falling back to latest API instance:', err);
            // fall back to latest api – event ordering may be off for a single block but prevents hard crash
            events = await this.substrateClient.query.system.events();
        }
